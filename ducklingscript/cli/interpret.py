...
from quackinter import Interpreter as QuackInterpreter, Config as QuackConfig, Command as QuackCommand
...
def interpret(
    ...
    delay: Annotated[
        int, typer.Option(help="How long in milliseconds to wait before we run the script.")
    ] = 1000,
):
    """
    ...
    with Progress() as progress:
        ...
        compiled = compile_comp.prepare_and_compile(filename, compile_options=CompileOptions(**config))
        ...
        try:
            ...
            interpreter = QuackInterpreter([create_between_lines(progress, main_task)], config=quack_config)
            interpreter.interpret_text("\n".join(ducky))
        except FailSafeException:
            print("[bold red]Failsafe triggered by putting mouse in the corner of the screen. Exiting...[/bold red]")
