...
class ConditionalOperator(Operator):
    ...
    def __find_operator(self, findable: str) -> bool:
        for i in self.operators:
            if i.startswith(findable): return True
        return False
    ...
    def addCharToToken(self, char: str) -> Token.isToken:
        ...
        if not self.pos_oper and not found:
            ...
            if self.pos_oper in self.operators:
                return Token.isToken.FALSE # Current char is not this operator, but previous one was
            else:
                return Token.isToken.RESET_CONTINUE # Nothing matches up, it is not this operand
