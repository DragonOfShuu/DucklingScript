...
class DocumentParser:
    ...
    def tab_char(self):
        if self._tab_char is None:
            raise ValueError(
                "Tab character has not been instantiated, yet has been called upon."
            )
    ...
    def _remove_tabs(
        ...
        tab_count_override: int | None = None,
    ):
    ...
    def _sync_stack(
        self, next_line: PreLine, current_tabination: int, new_tabination: int
    ) -> NoReturn | None:
        """
        Sync the current stack of lines with the new tabination level.
        """
    ...
    def next(
        self, options: DocumentParserNextOptions = DocumentParserNextOptions()
    ) -> PreLine | None:
        while True:
            ...
            self._sync_stack(next_line, current_tabination, new_tabination)
